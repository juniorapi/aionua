    ...
    document.querySelectorAll('.schedule-time').forEach(function(cell) {
        ...
        const convertedLines = lines.map(function(line) {
            ...
            if (match) {
                ...
                const todayNewZone = moment().tz("Asia/Kuwait").format("YYYY-MM-DD"); 
                let startNewZone = moment.tz(`${todayNewZone} ${startStr}`, "YYYY-MM-DD HH:mm", "Asia/Kuwait");
                let endNewZone = moment.tz(`${todayNewZone} ${endStr}`, "YYYY-MM-DD HH:mm", "Asia/Kuwait");
